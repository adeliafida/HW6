#Title: Homework 6
#Author: Adelia Fida
#Group members: Diep Luu,Bolina Oxilius
#Date: 11/04/2021
...
levels(vaxx_factor)
#for some reason we got an error about unexpected symbol for this
...
#For this subset we decided to use the region South instead of Northeast in the example.
...
#For this model, we wanted to focus on the marital status, if they're Hispanic or not, education level, their birth year and gender
#The estimates for education levels from some hs to bach degree are all less than 2 so the liklihood of them getting vaccinations is high. However, the education degree of adv deg is higher than 2 and the likelihood would be lower. 
...
#For some reason, every time we use the coding, there is always an error of one thing not found. 
